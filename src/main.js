...
gui.add(Controls, 'layout', 'custom')
...
const twitterVue = window.twitterVue = new Vue({render: h => h(App)}).$mount();

// handle routing (use Vue's reactive properties to update the DOM)
// function onHashChange() {
//     const visibility = window.location.hash.replace(/#\/?/, '')
//     if (filters[visibility]) {
//         twitterVue.visibility = visibility as any
//     } else {
//         window.location.hash = ''
//         twitterVue.visibility = 'all'
//     }
// }
...
const twitterLayer = window.twitterLayer = new WebLayer3D(twitterVue.$el, {
...
function toggleShadows(enabled) {
    if (enabled) {
        twitterLayer.traverseLayers(layer => {
            layer.contentMesh.material = makeShadowMaterial(layer)
        })
...
function toggleDOM(enabled) {
    ...
    const containerStyle = twitterLayer.element.parentElement.style;
    if (enabled) {
        containerStyle.top = ''
        ...
        twitterLayer.needsRefresh = true
    } else {
...
twitterLayer.interactionRays = mouseRays
...
function redirectEvent(evt) {
    const hit = twitterLayer.hitTest(raycaster.ray)
    if (hit) {
...
function animate() {
    ...
    if (renderer.vr.enabled && renderer.vr.getDevice()) {
        ...
        twitterLayer.position.lerp(todoLayerTargetPosition, lerpValue)
        twitterLayer.quaternion.slerp(todoLayerTargetQuaternion, lerpValue)
        twitterLayer.interactionRays = immersiveRays
    } else {
        ...
        twitterLayer.position.set(0, 0, 0)
        twitterLayer.quaternion.set(0, 0, 0, 1)
        twitterLayer.interactionRays = mouseRays
    ...
    // console.log('hi')
    // twitterLayer.contentOpacity.target = 0
    const wrapperLayer = twitterLayer.querySelector('.wrapper')
    console.log(wrapperLayer)
    // const tweetLayer = twitterLayer.querySelector('.main-body')
    // // const mainLayer = todoLayer.querySelector('.todoapp')
    // wrapperLayer.position.set(-0.4, 0.05, 0)
    // tweetLayer.position.set(-0.2, -0.05, 1)
        // mainLayer.position.set(0.2, 0, 0)
        // mainLayer.contentOpacity.target = 0
    // } else {
    //     twitterLayer.position.z = 0
    // }
